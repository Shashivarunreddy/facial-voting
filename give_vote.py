...
# Function to count total votes and save to a new file
def count_votes_and_save():
    if not os.path.isfile("Votes.csv"):
        print("No votes have been recorded yet.")
        return
    
    # Read the Votes.csv file and count votes
    with open("Votes.csv", "r") as csvfile:
        ...
        for row in reader:
            if len(row) > 1:  # Ensure the row has at least two columns
                votes.append(row[1])  # Extract the second column (Vote)
    ...
    # Save the counts to VoteSummary.csv
    with open("VoteSummary.csv", "w", newline="") as summaryfile:
        writer = csv.writer(summaryfile)
        writer.writerow(["Party/Option", "Total Votes"])
        for option, count in vote_count.items():
            writer.writerow([option, count])
    
    print("\nVote summary has been saved to 'VoteSummary.csv'.")
    return vote_count

   

# Initialize video capture and load face recognition model
...
while True:
    ...
    if output is not None:
        ...
        if k == ord('1'):
            ...
            count_votes_and_save()  # Save total votes after voting
            break
    ...
    if k == ord('s'):  # Press 's' to save total votes manually
        count_votes_and_save()
